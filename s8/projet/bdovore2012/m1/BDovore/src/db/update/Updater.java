...
public class Updater {
    ...
    /**
     * Execute la mise à jour d'une table : <br> - Récupère les entrées
     * manquante <br> - Parse le CSV pour en produire des requêtes <br> - INSERT
     * dans la base
     *
     * @param table
     * @param reponse
    ...
    /**
     * Génère un batch de
     * <code>limit</code> insertions SQL depuis le buffer CSV
     *
     * @param table
     * @param buffer
     ...
     *
     * - Traite le CSV transmis par le WS - parcourt toutes les editions contenu
     * dans la table BD_USER ainsi que toutes les transactions ( meme requete)
     * pour chaque édition on regarde si elle existe dans le csv et on effectue
     * le traitement approprié selon la valeur de la transaction associée (ou si
     * pas de transaction pour cet id). - Une fois le traitement effectué on
     * supprime la ligne correspondant à l'id traité dans le CSV. - Une fois
     * parcourue toutes les editions de BD_USER on regarde s'il reste des ligne
     * dans le csv et éventuellement on traite les cas restant.
     *
     *
     * @throws Exception
     *
    public void synchronizeUserAccount() throws Exception {
        ...
        while (rs.next()) {
            ...
            // On récupère l'édition a la ligne i si elle existe dans le csv
    ...
    /**
     * Compare les paramètres (Les flags) d'une édition entre la base locale et
     * le site
     *
     * @param rs edition courante de la base locale
     * @param lineEd ligne du csv courante (qui correspond à l'édition courante
    ...
    /**
     * Renvoie un String[] correspondant à l'édition a la ligne passée en
     * parametre sous la forme : (idEdition, FLG_P, FLG_D, FLG_A, Date)
     *
